...
const DEFAULT_KEYPAIR_FILE = path.join(DEFAULT_CONFIG_PATH.toString(), 'lobby-creator.json')
...
export const getDefaultSolanaWallet = ():Keypair => {
  const solanaConfig = execSync('solanax config get --output json', { encoding: 'utf-8' });
