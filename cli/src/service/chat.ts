...
async function getOrCreateDirectChannel(wallet: Keypair, withDID: string):Promise<Channel> {
  try {
...
export const chat = async (withDID: string, incomingMessages: Observable<string>):Promise<Observable<Message>> => {
