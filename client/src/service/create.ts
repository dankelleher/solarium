...
import {didToPublicKey, ExtendedCluster, isKeypair, pubkeyOf} from "../lib/util";
 ...
 * @param cluster
 */
const getOrCreateDID = async (owner: PublicKey, payer: Keypair | PublicKey, signCallback: SignCallback, cluster?: ExtendedCluster): Promise<DIDDocument> => {
  try {
 ...
 * @param cluster
 */
export const create = async (owner: PublicKey, payer: Keypair | PublicKey, signCallback?: SignCallback, cluster?: ExtendedCluster): Promise<Inbox> => {
  ...
  const didForOwner = await getOrCreateDID(owner, payer, createSignedTx, cluster)
  const didKey = didToPublicKey(didForOwner.id)

  console.log(`Creating inbox for DID: ${didForOwner.id}`);

  const inboxAddress = await SolariumTransaction.createInbox(pubkeyOf(payer), didKey, createSignedTx, cluster);
