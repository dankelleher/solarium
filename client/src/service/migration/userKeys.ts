...
const hasOldCEKAccountForChannel = async (did: string, channel: PublicKey, cluster?: ExtendedCluster): Promise<boolean> => {
  const didKey = didToPublicKey(did);
  const oldCEKAccountAddress = await getOldCekAccountAddress(didKey, channel);
  
  const account = await getConnection(cluster).getAccountInfo(oldCEKAccountAddress);
  return (!!account && account.data.length > 0);
}

const requiresMigration = async (did: string, channels: PublicKey[], cluster?: ExtendedCluster): Promise<boolean> => {
  ...
  const hasOldCekAccountForChannelsPromises = channels.map(
    channel => hasOldCEKAccountForChannel(did, channel, cluster)
  );

  // resolve the promises from above, and check if any of them are true
  return (await Promise.all(hasOldCekAccountForChannelsPromises)).some(identity);
}


function migrateChannel(details: MigrationDetails, userDetails: void, channel: PublicKey) {
  return undefined;
}

export const migrate = async (details:MigrationDetails): Promise<MigrationResult> => {
  const shouldMigrate = await requiresMigration(details.did, details.channels, details.cluster);
  ...
  details.channels.map(channel => migrateChannel(details, userDetails, channel))
}