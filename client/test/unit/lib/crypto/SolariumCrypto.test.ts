...
describe('SolariumCrypto', () => {
  ...
  beforeEach(() => {
    when(mockedResolve)
      .calledWith(aliceDID)
      .mockResolvedValue(dummyDIDDoc(aliceDID, aliceAccount.publicKey))
      .calledWith(bobDID)
      .mockResolvedValue(dummyDIDDoc(bobDID, bobAccount.publicKey))
      .calledWith(elviraDID)
      .mockResolvedValue(dummyDIDDoc(elviraDID, elviraAccount.publicKey));
  });
